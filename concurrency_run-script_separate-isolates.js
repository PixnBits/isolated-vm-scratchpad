...
(async function main() {
  try {
    const isolateA = new ivm.Isolate();
    ...
    const isolateB = new ivm.Isolate();
    const contextB = await isolateB.createContext();
    ...
    const intensiveCode = `
      ...
      log(primes)
    `;
    const intensiveScriptA = await isolateA.compileScript(intensiveCode);
    const intensiveScriptB = await isolateB.compileScript(intensiveCode);

    const aStart = process.hrtime.bigint();
    let aEnd;
    console.log('starting a', aStart);
    const a = intensiveScriptA.run(contextA).then(
      (result) => {
    ...
    const b = intensiveScriptB.run(contextB).then(
      (result) => {
    ...
    console.log('from previous runs this appears to show that scripts do run concurrently in separate isolates');
  } catch (error) {
    process.exitCode = 1;
    console.error(error);
  }
